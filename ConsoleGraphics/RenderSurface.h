...
namespace cg
{
	...
	public:
		...
		auto putCellUnsafe(int x, int y, CHAR_INFO color) -> void;
		auto putCell(int x, int y, CHAR_INFO color) -> void;
		...
		auto fill(CHAR_INFO color) -> void;
	...
	auto RenderSurface::getLine(int line) -> CHAR_INFO *
	{
		assert(
			line >= 0 &&
	...
	auto RenderSurface::getBuffer() -> CHAR_INFO *
	{
	...
	constexpr auto cg::RenderSurface::clamp(int & x, int & y) -> void
	{
		x = std::clamp(x, 0, static_cast<int>(m_surfaceSize.x) - 1);
		y = std::clamp(y, 0, static_cast<int>(m_surfaceSize.y) - 1);
	}

	constexpr auto cg::RenderSurface::on_surface(int x, int y) -> bool
	{
		return 
			x >= 0 &&
			y >= 0 &&
